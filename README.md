...
1. `code/batch_infer.py` — run the model over `data/images/` to produce `*_cp_masks.png`.
2. `code/grainsize.py` — measure a mask: ECD, Wentworth by count and by area (solidity >= 0.80).
3. `code/batch_grainsize.py` — per-slide summary across a folder (produces per_slide_grainsize.csv).
4. `code/recon_masks.py` — rebuild the RGLL ground truth from its delineation overlay.
5. `code/validate.py` — determinism check (SHA-256 of mask arrays across repeated runs).
6. `figures/Fig_production_grainsize.png` is generated from `results/per_slide_grainsize.csv`.

## Model weights (on Zenodo)
The fine-tuned model (Training2Samples, ~1 GB) is too large for GitHub and is archived on
Zenodo; download it and place it in `model/` (see `model/README.txt`). The Zenodo DOI is the
